...
experiment_id = '95214d24f4a6c5c47b38c50b253d3c4a'

data = pd.read_csv(os.path.join(folder, str(experiment_id)+'_pos_vel_ppp.dat'), sep='\t')
...
santos_zipped_data = ZipFile(os.path.join(folder, str(experiment_id)+'_santos_format.zip'), 'w')
...
with open(os.path.join(folder, str(experiment_id)+'_track_lenghts_matrix.txt'), 'w') as f:
    json.dump(tracks_and_lenghts, f, indent=0)
santos_zipped_data.write(os.path.join(folder, str(experiment_id)+'_track_lenghts_matrix.txt'),
                         arcname=str(experiment_id)+'_track_lenghts_matrix.txt')
santos_zipped_data.write(os.path.join(folder, str(experiment_id)+'_experiment_info.txt'),
                         arcname=str(experiment_id)+'_experiment_info.txt')
# Closing zip file
